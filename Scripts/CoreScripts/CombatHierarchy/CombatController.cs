...
namespace CMF
{
    public class CombatController : MonoBehaviour
    {
        ...
        public float LightningDistance, LightningHeight, BeamDistance, minionDistance, IceHeight;
        ...
        // Sound management
        public CombatSoundManager combatSounds;
        ...
        void Update()
        {
            ...
            spellTomeTimeRemaining-=Time.deltaTime;
            if(spellTomeTimeRemaining <= 0)
            {
                HideSpellTome();
            }
        }
        ...
        // Fire, wind, ice, lightning, light, void

        public GameObject spellTome;
        public Light tomeLight;
        public Color[] spellColors;
        public GameObject[] spellParticleEffects;
        public GameObject[] spellCircles;

        float spellTomeTimeRemaining = 0f;

        void ShowSpellTome(float time)
        {
            spellTome.SetActive(true);
            spellTomeTimeRemaining = time;
        }
        ...
        void ActivateMySpellCircle(int index)
        {
            for (int i = 0; i < spellCircles.Length; i++)
            {
                if (spellCircles[i] != null)
                {
                    if (i == index)
                    {
                        spellCircles[i].SetActive(true);
                        spellParticleEffects[i].SetActive(true);
                    }
                    else
                    {
                        spellCircles[i].SetActive(false);
                        spellParticleEffects[i].SetActive(false);
                    }
                }
            }
        }
        ...
        void UpdateMagicSystem()
        {
            ...
            if (fireballUnlocked && Input.GetKeyDown("f") && FC == 0f && PSM.mana >= 3)
            {
                ...
                FC = FireballCooldown;
                ShowSpellTome(2);
                ActivateMySpellCircle(0);
                combatSounds.PlaySound("Fire");
            }
            if (windBladesUnlocked&& Input.GetKeyDown("r") && WC == 0f && PSM.mana >= 5)
            {
                ...
                ShowSpellTome(2);
                ActivateMySpellCircle(1);
                combatSounds.PlaySound("Wind");
            }
            if (iceSpikesUnlocked&& Input.GetKeyDown("t") && IC == 0f && PSM.mana >= 4)
            {
                ...
                ShowSpellTome(2);
                ActivateMySpellCircle(2);
                combatSounds.PlaySound("Ice");
            }
            if (lightningUnlocked&& Input.GetKeyDown("v") && LC == 0f && PSM.mana >= 8)
            {
                ...
                ShowSpellTome(4);
                ActivateMySpellCircle(3);
                combatSounds.PlaySound("Lightning");
            }
            ...
            if (Input.GetKeyDown(KeyCode.Tab) && !weaponLogic.isSlashing)
            {
                if (!isEquipped)
                {
                    if (weapon.tag == "Sword")
                    {
                        ...
                        weapon.transform.localPosition = rightArm.transform.localPosition;
        ...
        void WeaponAttackLogic()
        {
            if (Input.GetMouseButtonDown(0) && isEquipped && !isMeleeAttack)
            {
                if (weapon.tag == "Sword")
                {
                    ...
                    combatSounds.PlaySound("Sword");
                }
                else if (weapon.tag == "Spear")
                {
                    weaponLogic.StartSpearThrust();
                    //weaponLogic.StartSwingThrow();
                    combatSounds.PlaySound("Spear");
                }
                if (weapon.tag == "Axe")
                {
                    //weaponLogic.StartThreeSixtySlashing();
        ...
        void StartFireball()
        {
            ...
            //GameObject f = Instantiate(spellCircles[0], ModelRoot.transform.position + ModelRoot.transform.forward * .5f + new Vector3(0f, 3f, 0f), cameraRootTransform.rotation);
        ...
        void StartIceSpikes()
        {
            for (int i = -2; i < 3; i++)
            {
                GameObject o = Instantiate(obj[2], ModelRoot.transform.position + ModelRoot.transform.forward * 3f + new Vector3(0f, IceHeight, 0f)/* + ModelRoot.transform.right * i * 2*/, ModelRoot.transform.rotation * Quaternion.Euler(0f, 180f, 0f));
                o.GetComponent<Rigidbody>().useGravity = false;
            }
        }
        void StartLightning()
